...
<head>
    ...
    <title>{{ __('messages.new_ticket_created') }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        <h2>{{ __('messages.new_ticket_created') }}</h2>

        <div class="ticket-info">
            <strong>{{ __('messages.subject') }}:</strong> {{ $ticket->subject }}<br>
            <strong>{{ __('messages.from') }}:</strong> {{ $ticket->user->name }} ({{ $ticket->user->email }})<br>
            <strong>{{ __('messages.date') }}:</strong> {{ $ticket->created_at->format('Y-m-d H:i') }}
        </div>

        <p>{{ __('messages.new_ticket_intro') . ' #' . $ticket->id }}</p>

        <p>{{ __('messages.thank_you') }},<br>{{ config('app.name') }}</p>

        <a href="{{ route('admin-tickets.view') }}" class="btn">{{ __('messages.visit_website') }}</a>
